...
int	is_valid_map_lines(char *line, t_game_info *data)
{
	...
	while (line[i])
	{
		...
		else if (line[i] != '1' && line[i] != '0' && line[i] != 'W' && line[i] != 'E'
		...
		while (map[x][y])
		{
			if(map[x][y] == '0'|| map[x][y] == 'N' || map[x][y] == 'S' || map[x][y] == 'E' || map[x][y] == 'W')
			{
				if (!map[x - 1] || !map[x - 1][y] ||  map[x - 1][y] == ' ')
					...
					return (printf("Error: map invalid5.\n"), ERROR);
			}
			y++;
		}
		x++;
...
{
	if (data->NO_file == NULL || data->SO_file == NULL || data->WE_file == NULL
		|| data->EA_file == NULL || data->floor[0] == -1
		|| data->cell[0] == -1)
	{
